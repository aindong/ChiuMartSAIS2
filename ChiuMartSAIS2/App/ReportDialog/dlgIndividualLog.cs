        ...
        private void populateLogByType()
        {
            try
            {
                using (MySqlConnection con = new MySqlConnection(conf.connectionstring))
                {
                    ...
                    {
                        sql = "SELECT l.id, l.username, l.quantity, l.created_date, l.log_type, u.unitDesc, p.productName, s.supplierName FROM logs as l INNER JOIN units as u ON l.unit_Id = u.unitId INNER JOIN products as p ON l.product_id = p.productId INNER JOIN supplier as s ON l.supplierId = s.supplierId WHERE l.log_type = 'transaction' AND s.supplierId = @relationId";
                    }
                    else if (logType == "balance")
                    {
                        sql = "SELECT l.id, l.username, l.price, l.created_date, l.paymentMethod, l.log_type, c.clientName FROM logs as l INNER JOIN client as c ON l.clientId = c.clientId WHERE l.log_type = 'balance' AND l.relationId = @relationId";
                    }
